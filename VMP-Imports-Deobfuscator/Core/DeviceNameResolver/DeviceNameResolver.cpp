...
void DeviceNameResolver::fixVirtualDevices()
{
    ...
    for (unsigned int i = 0; i < deviceNameList.size(); i++)
    {
        ...
        C_KernelWrapper::RtlInitUnicodeString(&unicodeInput, longCopy);
        InitializeObjectAttributes(&oa, &unicodeInput, 0, 0, 0);

        if(NT_SUCCESS(C_KernelWrapper::NtOpenSymbolicLinkObject(&hFile, SYMBOLIC_LINK_QUERY, &oa)))
        {
            ...
            if (NT_SUCCESS(C_KernelWrapper::NtQuerySymbolicLinkObject(hFile, &unicodeOutput, &retLen)))
            {
