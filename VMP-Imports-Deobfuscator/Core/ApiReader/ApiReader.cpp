...
#include "../Utils/StringConversion/StringConversion.h"
#include "../Pe/PeParser.h"
#include <unordered_map>

// @note: @colby57: This file is not changed.

using namespace ProcessAccessHelp;

std::unordered_map<DWORD_PTR, ApiInfo*> ApiReader::apiList;
std::unordered_map<DWORD_PTR, ImportModuleThunk>* ApiReader::moduleThunkList;
...
bool ApiReader::isModuleLoadedInOwnProcess(ModuleInfo* module)
{
	for (unsigned int i = 0; i < ownModuleList.size(); i++)
	{
		...
		{
			//wprintf(L"isModuleLoadedInOwnProcess :: %ws %ws\n",module->fullPath,ownModuleList[i].fullPath);
			return true;
		}
	}
	return false;
...
void ApiReader::readAndParseIAT(DWORD_PTR addressIAT, DWORD sizeIAT, std::unordered_map<DWORD_PTR, ImportModuleThunk>& moduleListNew)
{
...
bool ApiReader::addFunctionToModuleList(ApiInfo* apiFound, DWORD_PTR va, DWORD_PTR rva, WORD ordinal, bool valid, bool suspect)
{
	...
	std::unordered_map<DWORD_PTR, ImportModuleThunk>::iterator iterator1;
